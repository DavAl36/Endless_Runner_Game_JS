...
function createScene(){
    ...
    infoText.style.left = (sceneWidth/2)-120 + 'px';
...
loader.load('models/RobotExpressive.glb',
    function ( gltf ) {
        ...
        //robot.traverse( function (child) {console.log(child)} );  
        //helper = new THREE.SkeletonHelper(robot);
        //console.log(helper);
    },
...
function update(){
    ...
    if(diff_status == 1){rollingSpeed = 0.006;}
    if(diff_status == 2){rollingSpeed = 0.006;}
    ...
    //scene.visible=false;
    var diff = "Easy";
    if(diff_status == 0){diff = "Easy";  text_color = "orange"; camera.position.z = 7.5;}//orange
    if(diff_status == 1){diff = "Medium";text_color = "orange"; camera.position.z = 7.0;}//powderblue
    if(diff_status == 2){diff = "Hard";  text_color = "red";    camera.position.z = 6.5;}//red

    diff_memory = diff_status; // memorizes last diff value

    var testo = "(s)"+"&nbsp&nbsp&nbsp"+"Start" +"<br>"+"<br>"+
                "(d)"+"&nbsp&nbsp&nbsp"+"Difficulty: " + diff.toString()+ "<br>"+ "<br>";
    ...
    developText.style.color = "black";
    developText.style.fontFamily = "Comic Sans MS";
    developText.style.fontSize = "18pt";
    developText.innerHTML = " Tested on Mozilla Firefox 68.0.1  developed by Davide Aloisi";
